...
- **正交投影**：`hw.ortho(半高)` —— 半高是世界单位（写 30 就是上下各看 30 格），`0` 回透视。
  投影矩阵从写死的 `glFrustum` 改成按开关走 `glOrtho`；正交下物体不再近大远小，
  等距对齐视图、CAD 式三视图、编辑器顶部俯视图靠它。真渲实测：正交俯视网格全图等距
  （24 条竖线间距波动 **0.0%**），3/4 视角下网格线严格平行、无消失点；
  负数半高钳成 0，出图与透视路径**逐字节相同**（0 字节差异）。注意正交下 `hw.fov`
  不参与计算（视场由半高决定），`hw.near` / `hw.far` 照常裁剪
